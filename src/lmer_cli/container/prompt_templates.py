...
shell command and injects its output). pi has a native equivalent — markdown
*prompt templates* discovered from a per-user directory and invoked as slash
commands:

* pi: ``~/.pi/agent/prompts/*.md`` → ``/name``

Codex used to discover ``~/.codex/prompts/*.md`` as ``/prompts:name`` but
current releases have removed that mechanism in favor of skills. Its runner
therefore does not call this renderer; lmer uses plain-text instructions for
its start and follow-up control-plane messages instead.

Pi reads the same frontmatter fields lmer's command files already carry
...
* body: rewrite each leading-``!`` execution line into an instruction to
  ...
  prompt text, and pi then runs the command as an ordinary tool call;
* a body that never references its arguments gets a trailing
  ...
  arguments it expands to nothing). Pi also accepts ``$@`` as a synonym, but
  the renderer emits the clearer ``$ARGUMENTS`` spelling.
...
# Frontmatter keys shared by claude commands and pi prompt templates.
KEPT_FRONTMATTER_KEYS = ("description", "argument-hint")

# Argument placeholders pi expands; a body containing none of these
