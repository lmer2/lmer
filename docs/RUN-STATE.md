...
`name` is additive — **schema stays 1**, no migration: older readers ignore
unknown keys, and the schema guard only refuses *newer* schema numbers.
...
| `work session-start` | Hook-facing. Seed the run if absent, decide the resume brief *before* claiming, append `session_start`, claim `owner`, print the brief. Always exits 0. |
...
enum, `updated` timestamps, `owner` claims. Action: move `runs/<slug>/` →
`archive/<slug>/` (or set `status: archived`) for runs that are `complete`
or stale past a threshold.
