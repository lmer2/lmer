...
| `work release record receipt <name> [--url <url>] [--note "…"]` | Push/upload receipts; `<name>` ∈ `github-main-push`, `github-tag-push`, `actions-run`, `pypi`, `gitlab-tag-push`, `dep-refresh`; `--url` required for `actions-run`/`pypi` (records which run actually uploaded); receipts are re-recordable, history in events.jsonl; requires the tag to be recorded first (nothing can have been pushed yet otherwise) | `--url`, `--note` |
