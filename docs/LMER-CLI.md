...
- **`LMER_ANSWER`** - Answer to the run's recorded open question (the durable `open_question` field a previous session stored via `work state set --stop-reason=question --question "<text>"`). Set on the host via the `--answer` CLI flag (which wins over a host-set/`.env` value) and forwarded into the container by the host CLI. At session start, `work session-start` applies it automatically when the resolved run is actually stopped on a recorded question: it appends a `question_answered` event carrying both texts (secret-redacted), clears the question stop, and prints a resume brief that leads with the question+answer pair — so a fresh session resumes with clean context instead of reviving the session that asked. When the run has no open question the value is ignored (fail-soft). See docs/RUN-STATE.md §2.
