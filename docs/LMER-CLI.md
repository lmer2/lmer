...
- `POST /input` — body `{"data": "...", "append_newline": true}` writes to Claude's stdin. When `append_newline` is true and `data` does not already end with `\r`, a CR (`\r`) is appended so Claude's TUI treats it as Enter. A trailing `\n` is **not** a submit and does get a CR behind it (`"text\n"` → `"text\n\r"`): in raw mode `\n` only inserts a literal newline into the input box. To type a newline without pressing Enter, send `append_newline: false`.

  The Enter is sent **once**, with no follow-up bare-CR "nudges": a bare CR is a no-op only against an empty input box with no dialog on screen, and this endpoint is called mid-session, when a tool-permission prompt is exactly what may be up — a second CR would take that prompt's default. Since the supervisor writes to the PTY and cannot see whether the TUI registered the CR as a submit, the reply says so rather than implying delivery:
