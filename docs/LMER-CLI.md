...
- **`LMER_GATE_NO_FASTPATH`** - Kill switch for the gate's **text-only fast path** (issue #269). When a commit's staged change — or the commit range of a push of the current branch — touches nothing but prose (`*.md`, `*.rst`, `changelog.d/` fragments, `CHANGELOG.yaml`, `LICENSE`), `gate-check` runs the tests the project declared under `tests.text_diff_subset` in `.lmer/gate-check.yaml` (falling back to the work repo's `info/gate-check.yaml`) instead of the whole suite. The patterns are deliberately narrow because they are global while the compensating subset is per-repo: `.txt` is not prose (`requirements.txt`, `taskdef/*/instructions.txt`) and `docs/` is a location rather than a role (`docs/conf.py` executes), so both classify as code and take the full suite. The subset is the tests that READ text, not a "safe to skip" list — this repo's own tests read docs and rule files, so the honest skip list is empty — and `tests/test_gate_text_diff_subset.py` scans `tests/` and fails when the declaration is missing one. **That scan is a floor, not a proof:** it models repo-anchored `pathlib` composition, `joinpath` and `glob`/`rglob`, and is blind to `os.path.join`, f-string paths, `os.walk`, `glob.glob`, `iterdir`, cwd-relative `open('README.md')`, and tests that shell out to a repo script that reads a doc. A test written with one of those idioms must be added to the declaration by hand; a green scan does not certify the subset complete. A project that declares no subset gets the full suite, always, as does an unstaged or untracked working tree, a diff the classifier cannot resolve, a first push with no remote-tracking ref, and any push of an explicit `--ref`/`--tag` (the range the classifier can name is `HEAD` against the current branch's upstream, which is not what an explicit refspec pushes). Parsed via `get_bool_env`: set `LMER_GATE_NO_FASTPATH=1` (or `true`/`yes`) to run the full suite regardless. Forwarded into the container by the host CLI, since the gates run there.
